...
Source: https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/property
...
Indexed properties are commonly referred to as indexers. Indexers should be used only in APIs that provide access to items in a logical collection. For example, a string is a collection of characters, and the indexer on [System.String](/en-us/dotnet/api/system.string) was added to access its characters.
...
❌ AVOID indexers with parameter types other than [System.Int32](/en-us/dotnet/api/system.int32), [System.Int64](/en-us/dotnet/api/system.int64), [System.String](/en-us/dotnet/api/system.string), [System.Object](/en-us/dotnet/api/system.object), or an enum.
...
✔️ DO use the name `Item` for indexed properties unless there is an obviously better name (e.g., see the [Chars[]](/en-us/dotnet/api/system.string.chars) property on `System.String`).

In C#, indexers are by default named Item. The [IndexerNameAttribute](/en-us/dotnet/api/system.runtime.compilerservices.indexernameattribute) can be used to customize this name.
...
However, it is unlikely to be worth the overhead to raise such events for low-level APIs such as base types or collections. For example, [List<T>](/en-us/dotnet/api/system.collections.generic.list-1) would not raise such events when a new item is added to the list and the `Count` property changes.
