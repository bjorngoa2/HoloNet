...
A mutable type is a type with instances that can be modified after they are instantiated. For example, arrays, most collections, and streams are mutable types, but [System.Int32](/en-us/dotnet/api/system.int32), [System.Uri](/en-us/dotnet/api/system.uri), and [System.String](/en-us/dotnet/api/system.string) are all immutable. The read-only modifier on a reference type field prevents the instance stored in the field from being replaced, but it does not prevent the field’s instance data from being modified by calling members changing the instance.
