...
Source: https://learn.microsoft.com/en-us/dotnet/standard/design-guidelines/dependency-properties
...
✔️ DO inherit from [DependencyObject](/en-us/dotnet/api/system.windows.dependencyobject), or one of its subtypes, when implementing dependency properties. The type provides a very efficient implementation of a property store and automatically supports WPF data binding.

✔️ DO provide a regular CLR property and public static read-only field storing an instance of [System.Windows.DependencyProperty](/en-us/dotnet/api/system.windows.dependencyproperty) for each dependency property.

✔️ DO implement dependency properties by calling instance methods [DependencyObject.GetValue](/en-us/dotnet/api/system.windows.dependencyobject.getvalue) and [DependencyObject.SetValue](/en-us/dotnet/api/system.windows.dependencyobject.setvalue).
...
A classic example of an attached property is the [Grid.Column](/en-us/dotnet/api/system.windows.controls.grid.column#system-windows-controls-grid-column) property. The property represents Button’s (not Grid’s) column position, but it is only relevant if the Button is contained in a Grid, and so it's "attached" to Buttons by Grids.
...
❌ DO NOT implement change notification logic in dependency property accessors. Dependency properties have a built-in change notifications feature that must be used by supplying a change notification callback to the [PropertyMetadata](/en-us/dotnet/api/system.windows.propertymetadata).
