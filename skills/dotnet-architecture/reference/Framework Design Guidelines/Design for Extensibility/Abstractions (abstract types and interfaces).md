...
An abstraction is a type that describes a contract but does not provide a full implementation of the contract. Abstractions are usually implemented as abstract classes or interfaces, and they come with a well-defined set of reference documentation describing the required semantics of the types implementing the contract. Some of the most important abstractions in the .NET Framework include [Stream](/en-us/dotnet/api/system.io.stream), [IEnumerable<T>](/en-us/dotnet/api/system.collections.generic.ienumerable-1), and [Object](/en-us/dotnet/api/system.object).
